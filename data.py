...
# cifar100 非独立同分布数据集
def get_CIFAR100(alpha: float, num_clients: int) -> Tuple[
    List[DataLoader], List[DataLoader], List[int]]:
    transform = transforms.Compose([
        transforms.RandomHorizontalFlip(),
        transforms.RandomCrop(32, padding=4),
        transforms.ToTensor(),
        transforms.Normalize((0.5071, 0.4867, 0.4408), (0.2675, 0.2565, 0.2761)),  # CIFAR-100 recommended normalization
    ])
    ...
    # Shared test loader for all clients
    shared_test_loader = DataLoader(test_dataset, batch_size=256, shuffle=False)
...
def get_FEMNIST(numOfClients):
    ...
    raw_train, raw_test = datasets.EMNIST(cache_dir='../data/femnist', only_digits=False)
...
def get_EMNIST(num_clients):
    ...
    # emnist_train = datasets.EMNIST('./data/EMNIST_BYCLASS', split='byclass', train=True, download=False, transform=transform)
    # emnist_test = datasets.EMNIST('./data/EMNIST_BYCLASS', split='byclass', train=False, download=False, transform=transform)
    emnist_train = datasets.EMNIST('./data/EMNIST', split='balanced', train=True, download=False, transform=transform)
    emnist_test = datasets.EMNIST('./data/EMNIST', split='balanced', train=False, download=False, transform=transform)
